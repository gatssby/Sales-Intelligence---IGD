// ==UserScript==
// @name         Verbo AI - Gemini Web Worker POC
...
(() => {
  ...
  function log(message, extra = undefined) {
    ...
    if (extra === undefined) {
      console.info("[Verbo Gemini Worker]", message);
    } else {
      console.info("[Verbo Gemini Worker]", message, extra);
    }
  }
...
${transcript}`;
  ...
  GM_registerMenuCommand("Verbo: configurar backend", configureBackend);
  GM_registerMenuCommand("Verbo: configurar token", configureToken);
  GM_registerMenuCommand("Verbo: ativar/desativar worker", () => {
    setEnabled(!isEnabled());
    setState(isEnabled() ? "ativado" : "desativado");
    if (isEnabled()) scheduleLoop(100);
  });
  GM_registerMenuCommand("Verbo: buscar job agora", () => void claimOnce());
  GM_registerMenuCommand("Verbo: renovar workerId", resetWorkerId);
  GM_registerMenuCommand("Verbo: mostrar configuração", () => {
