  ...
  return (
    <main className="admin-shell calls-shell">
      ...
      <section className="detail-metadata">
        ...
        <article className="metric-card"><p>{technical ? "Modelo final" : "Revisão humana"}</p><strong className="word-stat">{technical ? call.finalModel?.split("/").at(-1) ?? "—" : call.humanReviewRequested ? "Recomendada" : "Dispensada"}</strong><span className="metric-note">{technical ? call.escalated ? "Com escalation" : "Primary" : "Decisão operacional"}</span></article>
      </section>
      ...
      {analysis ? <>
        ...
        <section className="panel audit-card"><p className="eyebrow">Trilha de auditoria</p><h2>{technical ? "Execução oficial" : "Registro da análise"}</h2><dl>
          {technical ? <><div><dt>Rubrica</dt><dd>{call.rubricVersion}</dd></div><div><dt>Prompt</dt><dd>{call.promptVersion}</dd></div><div><dt>Schema</dt><dd>{call.schemaVersion}</dd></div><div><dt>Confidence policy</dt><dd>{call.confidencePolicyVersion ?? "policy histórica"}</dd></div><div><dt>Latency total</dt><dd>{call.latencyMs === null ? "—" : `${call.latencyMs} ms`}</dd></div><div><dt>Custo</dt><dd>{call.costUsd === null ? "—" : `$${call.costUsd.toFixed(6)}`}</dd></div><div><dt>Escalation reasons</dt><dd>{call.escalationReasons.join(", ") || "Nenhuma"}</dd></div></> : null}
          <div><dt>Analisada em</dt><dd>{formatDate(call.analyzedAt)}</dd></div>
        </dl>{technical ? <div className="attempt-list">{call.attempts.map((attempt) => <div key={attempt.attemptNumber}><b>#{attempt.attemptNumber} · {attempt.role}</b><span>{attempt.model} · {attempt.status} · {attempt.latencyMs ?? "—"} ms · {attempt.costUsd === null ? "custo indisponível" : `$${attempt.costUsd.toFixed(6)}`}</span></div>)}</div> : <p className="muted-copy">Detalhes de provider, custo e tentativas ficam na área Platform.</p>}</section>
