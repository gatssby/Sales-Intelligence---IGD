...
export async function POST(request: NextRequest) {
  ...
  const body = await request.json().catch(() => null) as { kind?: unknown; subjectPersonId?: unknown } | null;
  ...
  try {
    const previewContext = await repository.resolvePreviewContext(actor, { kind, subjectPersonId });
    await repository.recordPreviewEvent(actor, "preview.started", previewContext.preview);
    await setPreviewCookie({ kind, subjectPersonId });
