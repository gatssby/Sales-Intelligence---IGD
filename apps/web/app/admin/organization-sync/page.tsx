...
export default async function OrganizationSyncPage() {
  ...
  const latest = runs[0];
  return <main className="admin-shell"><header className="admin-header"><div><p className="eyebrow">Admin · Sincronização</p><h1>Organization Sync</h1><p>Google Sheet read-only → candidate validado → PostgreSQL temporal.</p></div><div className="admin-header-actions"><a href="/organization">Organização</a><a href="/">Visão Geral</a></div></header><section className="panel"><div className="section-title"><div><h2>Status: {latest?.status ?? "sem execução"}</h2><p>Último sync: {formatDate(latest?.finished_at ?? null)} · linhas {latest?.rows_read ?? 0} · pessoas {latest?.valid_people ?? 0} · warnings {latest?.warning_count ?? 0}</p></div><SyncNowButton /></div>{latest?.rejection_reasons.length ? <p className="form-error">{latest.rejection_reasons.join(" · ")}</p> : null}</section><section className="panel"><h2>Histórico recente</h2><div className="team-table-wrap"><table className="team-table"><thead><tr><th>Concluído</th><th>Status</th><th>Revision</th><th>Rows</th><th>People</th><th>Warnings</th><th>Resumo</th></tr></thead><tbody>{runs.map((run) => <tr key={run.id}><td>{formatDate(run.finished_at)}</td><td>{run.status}</td><td>{run.spreadsheet_revision ?? "—"}</td><td>{run.rows_read}</td><td>{run.valid_people}</td><td>{run.warning_count}</td><td>{JSON.stringify(run.summary)}</td></tr>)}</tbody></table></div></section></main>;
}
