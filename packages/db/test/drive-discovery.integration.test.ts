...
integration("Drive discovery registry is canonical, recoverable and concurrency-safe", async () => {
  ...
  try {
    ...
    assert.deepEqual(await discovery.listSourceIdsWithIneligibleCalls(), [source.sourceId]);
    assert.ok(await discovery.linkDocumentToExistingCall(original.documentId, true));
    assert.equal((await sql<{ analysis_eligible: boolean }[]>`select analysis_eligible from calls where id=${linked.callId}`)[0].analysis_eligible, true);
    assert.equal((await sql`select 1 from analysis_jobs where call_id=${linked.callId}`).length, 1);
    assert.deepEqual(await discovery.listSourceIdsWithIneligibleCalls(), []);
