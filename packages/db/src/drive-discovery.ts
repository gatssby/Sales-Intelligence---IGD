...
export class PostgresDriveDiscoveryRepository {
  ...
  async listSourceIdsWithIneligibleCalls(): Promise<string[]> {
    const rows = await this.sql<{ source_id: string }[]>`
      select distinct dds.source_location_id source_id
      from drive_document_sources dds
      join drive_documents d on d.id=dds.drive_document_id
      join calls c on c.id=d.call_id
      join source_locations sl on sl.id=dds.source_location_id
      where dds.active=true and c.analysis_eligible=false
        and sl.provider='google_drive' and sl.source_type='folder'
        and sl.active=true and sl.registration_status='enabled'
      order by dds.source_location_id
    `;
