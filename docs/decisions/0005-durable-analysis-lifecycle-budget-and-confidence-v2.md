...
- O claim permanece atômico com `FOR UPDATE SKIP LOCKED`. Um restart retoma escalation já decidida ou finaliza um resultado persistido, sem repetir o primary.
- Toda request paga exige uma reserva em `ai_cost_reservations`, sob lock da conta global. O attempt e o custo real de `providerMetadata.gateway.cost` são liquidados na mesma transação.
